...
{
	float intlow = (float)low;
	float inthigh = (float)high + 0.999f;
	return (int)(Ranf(low, high));
}

// defining void TimeOfDaySeed()
...
void TimeOfDaySeed()
{
	...
	time_t timer;
	time(&timer);
	srand((unsigned int)(difftime(timer, mktime(&t))));
}
		...
		for (int n = 0; n < NUMTRIALS; n++)
		{
			...
			else
			{
				...
				else
				{
					...
					float t1 = (-B + sqrt(disc)) / (2.f * A);	// time to intersect high ground
					float t2 = (-B - sqrt(disc)) / (2.f * A);	// time to intersect high ground
